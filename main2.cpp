...
// #include "lib/graph2/include/window.hpp"
// #include "lib/graph2/include/wrappers/glfw.hpp"
// #include "lib/graph2/include/shaders.hpp"
...
static unsigned int program, vao, vbo = -1;
...
static void create_popup(uint32_t serial) {
	...
	popup_egl_surface = (wlr_egl_surface*)eglCreatePlatformWindowSurfaceEXT(
...
static void handle_global(void *data, struct wl_registry *registry,
	...
	if (strcmp(interface, wl_compositor_interface.name) == 0) {
		...
		if (output != UINT32_MAX) {
			if (!wl_output) {
				wl_output = (struct wl_output*)wl_registry_bind(registry, name,
		...
		layer_shell = (zwlr_layer_shell_v1*)wl_registry_bind(registry, name,
...
int main(int argc, char **argv) {
	...
	egl_init(display);
    PFNGLGETSTRINGPROC pp = (PFNGLGETSTRINGPROC)eglGetProcAddress("glGetString");
    if (!pp) {
        printf("!p D:\n");
    }
    const unsigned char *response = pp(GL_VERSION);
    printf("%s\n", response);
    response = pp(GL_VENDOR);
    printf("%s\n", response);

    const char * clientAPI= eglQueryString(EGL_NO_DISPLAY, EGL_CLIENT_APIS);
    if (clientAPI != NULL) {
        printf("Supported client APIs: %s\n", clientAPI);
    } else {
        printf("Failed to retrieve supported client APIs\n");
    }
	...
	assert(egl_window);
	egl_surface = (wlr_egl_surface*)eglCreatePlatformWindowSurfaceEXT(
